/**
 * Created by zhangle on 2017/4/12.
 */
...
function PromotionRecommend({promotion,dispatch}) {
  console.log(promotion,23);
  let str =''
  let stt=''
  if(promotion.record.isRecommend ==2){
    str ="推荐促销活动"
    stt='确认开启'
  }else {
    str='取消推荐促销活动'
  ...
  const onChange =()=>{
    if(promotion.record.isRecommend ==2){
      console.log(promotion.record.retCode,1111111)
      dispatch({
        type: 'promotion/recommendPromption',
        payload: {
          promotionNo:promotion.record.promotionNo,
        },
      });
      if(promotion.record.retCode==1){
      ...
      dispatch({
        type: 'promotion/cancelRecommendPromption',
        payload: {
          promotionNo:promotion.record.promotionNo,
        },
      });
      if(promotion.retCode==1){
...
export default connect(mapStateToProps)(PromotionRecommend)
